...
-(void)drawCircle
{
    ...
    {
        [view setBackgroundColor:self.darkGreen];
    }
...
-(void)readLastLocation
{
    ...
    self.lastDateIN =@"";
    [self openDB];
    NSString *sql =@"select * from location as a,(select locationID,max(eventid),DateTimeIN,DateTimeOut from event) as b where a.locationID=b.locationID";
...
-(void)createTables
{
    ...
    [self openDB];
    sql=@"CREATE TABLE IF NOT EXISTS 'Event' (EventID INTEGER PRIMARY KEY AUTOINCREMENT,'LocationID' TEXT,'DateTimeIN' TEXT,'DateTimeOut' TEXT,'LatIN' TEXT,'LonIN' TEXT,'LatOUT' TEXT,'LonOUT' TEXT,'sendEmailIN' TEXT,'sendEmailOUT' TEXT)";
    if (sqlite3_exec(appDB, [sql UTF8String], NULL, NULL, &err) != SQLITE_OK) {
    }
    sql=@"CREATE TABLE IF NOT EXISTS 'Location' (locationID INTEGER PRIMARY KEY AUTOINCREMENT,'Description' TEXT,'Address' TEXT,'City' TEXT,'State' TEXT,'Zip' TEXT,'NLat' TEXT,'NLon' TEXT,'WLat' TEXT,'WLon' TEXT,'SLat' TEXT,'SLon' TEXT,'ELat' TEXT,'ELon' TEXT,'emailTo' TEXT,emailFrom TEXT,messageIN TEXT,messageOUT TEXT,'enableLocation' Boolean)";
    if (sqlite3_exec(appDB, [sql UTF8String], NULL, NULL, &err) != SQLITE_OK) {
    }
    ...
    if([sentEmailIN isEqualToString:@"0"])
        {
            ...
            {
                dateFromString = [dateFormat dateFromString:lastDateIN];
...
-(void)createAnimation2
{
    ...
    self.animationImageView.animationDuration = 50;
    NSMutableArray *starImageArray = [@[] mutableCopy];
    NSString *number;
    for (int i = 0; i < 41; i++) {
        if(i < 10)
        {
            number= [NSString stringWithFormat:@"0%i",i];
        }
        else
        {
            number= [NSString stringWithFormat:@"%i",i];
        }
        UIImage *starImage = [UIImage imageNamed:[NSString stringWithFormat:@"circle%@", number]];
        [starImageArray addObject:starImage];
    }
    self.animationImageView.animationImages = starImageArray;
}
...
- (void)locationManager:(CLLocationManager *)manager didUpdateLocations:(NSArray *)locations
{
    ...
    self.lblSpeed.text= [NSString stringWithFormat:@"%f",speed ];
    
   NSLog(@"%@ %f", [locations lastObject],speed);
...
- (void)locationUpdate:(CLLocation *)location {
    self.lblSpeed.text = [NSString stringWithFormat:@"SPEED: %f", [location speed]];
...
- (void)locationError:(NSError *)error {
    self.lblSpeed.text = [error description];
}
